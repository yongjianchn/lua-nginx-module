...
static void ngx_http_pipe_put_error(ngx_http_lua_pipe_ctx_t *pipe_ctx,
...
/* signals below are handled specially in Nginx.
 * We need to reset them for the spawned child processes */
#define SIG(name)  { (SIG##name),  "SIG"#name }
...
ngx_http_lua_pipe_add_signal_handler(ngx_cycle_t *cycle)
{
    ...
    if (ngx_http_lua_signalfd < 0) {
        ngx_log_error(NGX_LOG_ERR, cycle->log, ngx_errno,
                      "lua pipe create new signalfd instance failed");
        return NGX_ERROR;
    }
...
#   if !(NGX_HTTP_LUA_HAVE_PIPE2)
    if (ngx_nonblocking(ngx_http_lua_read_sigfd) == -1) {
        ngx_log_error(NGX_LOG_ERR, cycle->log, ngx_errno,
                      "lua pipe " ngx_nonblocking_n " SIGCHLD fd failed");
        goto failed;
    }
...
#if !(NGX_HTTP_LUA_HAVE_SIGNALFD)
static void
ngx_http_lua_pipe_sigchld_handler(int signo, siginfo_t *siginfo, void *ucontext)
{
    ...
    for ( ;; ) {
        ...
        ngx_log_debug0(NGX_LOG_DEBUG_EVENT, ngx_cycle->log, err,
                       "lua pipe write sigfd was interrupted");
    }
            ...
            if (node != NULL) {
                ...
                if (WIFSIGNALED(status)) {
                    ...
                    ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                                  "unknown exit status %d with process %P",
                                  status, pid);
...
ngx_http_lua_ffi_pipe_spawn(ngx_http_lua_ffi_pipe_proc_t *proc,
    ...
    u_char *errbuf, size_t *errbuf_size)
{
#ifndef HAVE_SOCKET_CLOEXEC_PATCH
    *errbuf_size = ngx_snprintf(errbuf, *errbuf_size,
                                "missing socket cloexec patch in "
                                "the nginx core")
                   - errbuf;
    return NGX_ERROR;
    ...
    if (pid == 0) {
        ...
        for (sig = ngx_signals; sig->signo != 0; sig++) {
            ...
            if (sigaction(sig->signo, &sa, NULL) == -1) {
                ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, ngx_errno,
                              "lua pipe reset signal handlers for %s "
                              "in child failed", sig->signame);
                exit(EXIT_FAILURE);
            }
        }
        ...
        if (sigprocmask(SIG_SETMASK, &set, NULL) != 0) {
            ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, ngx_errno,
                          "lua pipe reset signal mask failed");
            exit(EXIT_FAILURE);
        }
        ...
        for (i = 0; i < ngx_cycle->listening.nelts; i++) {
            if (ngx_close_socket(ls[i].fd) == -1) {
                ngx_log_error(NGX_LOG_EMERG, ngx_cycle->log, ngx_socket_errno,
                              "lua pipe " ngx_close_socket_n " %V failed",
                              &ls[i].addr_text);
            }
        }
        ...
        if (errlog_fd == STDERR_FILENO) {
            ...
            if (temp_errlog_fd == -1) {
                ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, ngx_errno,
                              "dup errlog fd failed");
                exit(EXIT_FAILURE);
            }

            if (ngx_cloexec(temp_errlog_fd) == -1) {
                ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, ngx_errno,
                              "new errlog fd " ngx_cloexec_n " failed");
            }

            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                           "lua pipe dup old errlog fd %d to new fd %d",
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                       "lua pipe spawn process:%s pid:%P", file, getpid());

        if (execvp(file, (char * const *) argv) == -1) {
            ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, ngx_errno,
                          "lua pipe execvp() failed while executing %s", file);
        }
    ...
    ngx_event_t *ev, int forced)
{
    if (ev->handler != ngx_http_lua_pipe_dummy_event_handler && !forced) {
        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                       "lua pipe can not close fd:%d without "
    ...
    u_char *errbuf, size_t *errbuf_size)
{
    ...
    if (rc != NGX_OK) {
        *errbuf_size =
            ngx_snprintf(errbuf, *errbuf_size, "pipe busy writing") - errbuf;
        return NGX_ERROR;
    }
    ...
    u_char *errbuf, size_t *errbuf_size)
{
    ...
    if (rc != NGX_OK) {
        *errbuf_size =
            ngx_snprintf(errbuf, *errbuf_size, "pipe busy reading") - errbuf;
        return NGX_ERROR;
    }
    ...
    u_char *errbuf, size_t *errbuf_size)
{
    ...
    if (pipe->merge_stderr) {
        ...
        rc = ngx_http_lua_pipe_close_stderr(pipe, 0);
    }
...
static void
ngx_http_pipe_put_error(ngx_http_lua_pipe_ctx_t *pipe_ctx, u_char *errbuf,
    size_t *errbuf_size)
{
    ...
    default:
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0, "unexpected err type: %d",
                      pipe_ctx->err_type);
...
ngx_http_lua_pipe_read_line(void *data, ssize_t bytes)
{
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "lua pipe read line");
    ...
    ngx_http_lua_pipe_ctx_t *pipe_ctx)
{
    ...
    for ( ;; ) {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                       "lua pipe try to read data %uz pipe:%p", size, pipe_ctx);
...
error:
    ...
    if (pipe_ctx->bufs_in) {
        ngx_http_lua_pipe_put_data(pipe, pipe_ctx, buf, buf_size);
        ngx_http_pipe_put_error(pipe_ctx, errbuf, errbuf_size);
        return NGX_DECLINED;
    }
 ...
 * ngx_http_lua_ffi_pipe_proc_write, so we omit most of the sanity check already
 * done in ngx_http_lua_ffi_pipe_proc_write.
 */
    ...
    lua_State *L, int from_stderr)
{
    ...
    pipe = proc->pipe;
    if (from_stderr && !pipe->merge_stderr) {
        pipe_ctx = pipe->stderr_ctx;
...
static int
ngx_http_lua_pipe_write_retval(ngx_http_lua_ffi_pipe_proc_t *proc, lua_State *L)
{
    int                              rc;
    ngx_msec_t                       timeout;
    ngx_event_t                     *wev;
